...
#ifdef USE_SIXTY_DEGREES
    vector3d pq = vector3d(p) - vector3d(q);
    vector3d rq = vector3d(r) - vector3d(q);
    return (pq * rq)/sqrt(pq.sq_length()*rq.sq_length()) > 0.49;
#endif
...
bool PLCx::midsplitMissingEdge(uint32_t mei) {
    if (!is_missing_PLCedge(mei)) return false;
    assert(edges[mei].type == no_acute_ep);

    uint64_t ct = delmesh.inc_tet[edges[mei].ep[0]] << 2;
    middleEdgeSplit(mei);
    delmesh.insertExistingVertex((uint32_t)delmesh.vertices.size() - 1, ct);
...
//// 
void PLCx::segmentRecovery_Grooming(bool quiet)
{
    // 1) Create an LFS vector init to DBL_MAX
    std::vector<double> lfs(delmesh.numVertices(), DBL_MAX);

    // 2) for each delmesh edge calculate sqlen and if < LFS(v), where v endpoint, LFS(v) = sqlen
    for (uint64_t tet_i = 0; tet_i < delmesh.numTets(); tet_i++) {
        const uint32_t* tn = delmesh.getTetNodes(tet_i<<2);
        if (tn[3] == INFINITE_VERTEX) continue;
        for (uint32_t i = 0; i < 4; i++)
            for (uint32_t j = i + 1; j < 4; j++) {
                const uint32_t v1i = tn[i], v2i = tn[j];
                const vector3d v1 = delmesh.vertices[v1i];
                const vector3d v2 = delmesh.vertices[v2i];
                const double sql = v1.dist_sq(v2);
                if (lfs[v1i] > sql) lfs[v1i] = sql;
                if (lfs[v2i] > sql) lfs[v2i] = sql;
            }
    }
    
    // 3) split each nonflat edge at sqrt(LFS(v1)/9) and sqrt(LFS(v2)/9)
    const size_t oes = edges.size();
    for (uint32_t ei = 0; ei < oes; ei++) {
        const PLCedge& e = edges[ei];
        if (e.isFlat()) continue;
        tripleEdgeSplit(ei, lfs[e.ep[0]], lfs[e.ep[1]]);
    }

    delmesh.removeDelTets();

    // 4) Iteratively bisect remaining missing edges
