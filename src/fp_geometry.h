...
inline pointType* createMidPoint(const pointType* v0, const pointType* v1)
{
	...
	// LL -> if endpoints coincide, create LNC with avg t; 
	//		 else at least an endpoint must coincide (assert), create BPT with avg t1 and t2
...
inline double createTetCurcumcenterPrecise(const pointType* v0, const pointType* v1, const pointType* v2, const pointType* v3, double* cc) {
	...
	return radius.get_d() / (dden*dden);
}


inline double createTetCurcumcenterPrecise(const pointType* v0, const pointType* v1, const pointType* v2, const pointType* v3, vector3d& cc)
{
	return createTetCurcumcenterPrecise(v0, v1, v2, v3, cc.c);
}
...
inline double createTetCurcumcenter(const pointType* v0, const pointType* v1, const pointType* v2, const pointType* v3, vector3d& cc)
{
	...
	if (pointType::inSphere(explicitPoint(cc.c[0], cc.c[1], cc.c[2]), *v0, *v1, *v2, *v3) < 0) 
		return createTetCurcumcenterPrecise(v0, v1, v2, v3, cc);
...
inline double getTetCircumSpherePrecise(const pointType* v[4], double ccc[3]) {
	vector3d cc;
	double radius = createTetCurcumcenterPrecise(v[0], v[1], v[2], v[3], cc);
	ccc[0] = cc.c[0];
	ccc[1] = cc.c[1];
	ccc[2] = cc.c[2];
...
// never called
// inline double minTetAngle(const pointType* v0, const pointType* v1, const pointType* v2, const pointType* v3) {
// 	double minang = 0.0;
// 	const vector3d v[4] = { v0, v1, v2, v3 };
// 	for (int i = 0; i < 4; i++) {  // <-- THIS CYCLE DOES NOT CHECK EACH FACE ANGLE!!
// 		for (int j = i+1; j < 4; j++) {
// 			for (int k = j+1; k < 4; k++) {
...
// 	const double mindihedral = minTetDihedralAngleCos(v0, v1, v2, v3);
// 	if (mindihedral > minang) minang = mindihedral;
// 	return 180.0 * (acos(minang) / M_PI);
// }

// Given the triangle of side length a,b,c
// returns the angle (degrees) between a,b
// NOTE. the algorithm is taken from "Miscalculating Area and Angles of a Needle-like Triangle" by Prof. W. Kahan
